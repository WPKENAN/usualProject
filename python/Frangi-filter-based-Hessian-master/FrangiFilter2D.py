...
def FrangiFilter2D(I):
    I=np.array(I,dtype=float)
    defaultoptions = {'FrangiScaleRange':(1,10), 'FrangiScaleRatio':2, 'FrangiBetaOne':0.5, 'FrangiBetaTwo':15, 'verbose':True,'BlackWhite':False};
...
if __name__ == "__main__":
    imagename="D:\github\\usualProject\python\Frangi-filter-based-Hessian-master\Screenshots\\18454-1.png"
    ...
    cv2.imshow('img',img)
    # print(np.max(img))
    # img=(img/(np.max(img)-np.min(img))*512).astype("int8")

    # print(img)
    cv2.imwrite(imagename+"result.png",img)
    cv2.waitKey(0)
