...
def readCsv(path):
    ...
    # df['SA'] = np.array([analize_sentiment(tweet) for tweet in df[1]])
    # print(df)
    # df['SA'] = np.array([1 for tweet in df[1]])
    # print(df.shape)
    data = df.values
...
def calPreLength(data,movie='GREEN BOOK',predate='1999-02-02'):
    ...
    SA=calSA(tmpdata,movie)
    # print(SA)

    return meanlength,count,SA
...
def drawLength(data):
    ...
    for movie in movie_date:
        print(movie)
        meanlength,count,SA=calPreLength(data,movie,movie_date[movie][0])
        meanlengths.append(meanlength)
        counts.append(count)
        revenue.append(movie_revenue[movie])
        SAs.append(copy.deepcopy(SA))
    meanlengths=np.array(meanlengths)
    counts=np.array(counts)
    revenue=np.array(revenue)
    writeSA("SA_pre.csv",SAs,revenue);
    ...
    SAs = []
    for movie in movie_date:
        meanlength, count, SA = calPrePostLength(data, movie, movie_date[movie][0],movie_date[movie][1])
        meanlengths.append(meanlength)
        counts.append(count)
        revenue.append(movie_revenue[movie])
        SAs.append(copy.deepcopy(SA))
    ...
    counts = np.array(counts)
    revenue = np.array(revenue)
    writeSA("SA_pre-post.csv", SAs, revenue);
    ...
    SAs = []
    for movie in movie_date:
        meanlength, count, SA = calPostLength(data, movie, movie_date[movie][1])
        meanlengths.append(meanlength)
        counts.append(count)
        revenue.append(movie_revenue[movie])
        SAs.append(copy.deepcopy(SA))
    ...
    counts = np.array(counts)
    revenue = np.array(revenue)
    writeSA("SA_post.csv", SAs, revenue);
    ...
    SAs = []
    for movie in movie_date:
        meanlength, count, SA = calAllLength(data, movie)
        meanlengths.append(meanlength)
        counts.append(count)
        revenue.append(movie_revenue[movie])
        SAs.append(copy.deepcopy(SA))
    ...
    counts = np.array(counts)
    revenue = np.array(revenue)
    writeSA("SA_total.csv", SAs, revenue);
