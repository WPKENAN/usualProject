...
def bayesAlgorithm(trainPath, testPath):
    ...
    print("accurary rate:", 100-float(rate) * 100 / float(total), "%")


def bayesAlgorithm2(trainPath, testPath):
    trainSet = readBunch(trainPath)
    testSet = readBunch(testPath)
    clf = MultinomialNB(alpha=0.001).fit(trainSet.tdm, trainSet.label)
    #alpha:0.001 alpha 越小，迭代次数越多，精度越高
...
def pre(str):
    # os.remove("test_segResult/");
    shutil.rmtree("test_segResult/")
    segText(str, "test_segResult/")  # 分词
    bunchSave("test_segResult/", "test_set.dat")
    getTestSpace("test_set.dat", "tfidfspace.dat", stopWordList, "testspace.dat")
    bayesAlgorithm2("tfidfspace.dat", "testspace.dat")


stopWordList = getStopWord("stop/stopword.txt")  # 获取停用词

def train():
    # 分词，第一个是分词输入，第二个参数是结果保存的路径
    segText("data/", "segResult/")
    bunchSave("segResult/", "train_set.dat")  # 输入分词，输出分词向量
    getTFIDFMat("train_set.dat", stopWordList, "tfidfspace.dat")  # 输入词向量，输出特征空间

    # 训练集
    segText("test/", "test_segResult/")  # 分词
    bunchSave("test_segResult/", "test_set.dat")
    getTestSpace("test_set.dat", "tfidfspace.dat", stopWordList, "testspace.dat")
    bayesAlgorithm("tfidfspace.dat", "testspace.dat")
