def readCsv(path1,path2):
    file=open(path1,encoding='utf8');#使用utf8编码格式打开csv
    contents=file.readlines();#全部读入
    contents=contents[1:len(contents)]#读入除了第一行之外的所有行
    ...
    MAX = float('inf')
    stationList = station.keys();#获取所有车站
    stationMap=[[MAX]*(len(station)+1) for i in range(len(station)+1)]#初始化地图
    ...
    for i in range(len(contents)):
        ...
        #初始化权值！！！！！！！！！！！！！
        stationMap[startStation][endStation]=1;
        stationMap[endStation][startStation] = 1;#无向图，权是excel
...
def floyd(station,stationMap):
    ...
    outfile=open('result.csv','w')#w 是写的意思,写csv
    outfile.write('stationName,stationId,shortest,path,pathName\n');
    for i in range(1,len(station)+1):
        for j in range(1,len(station)+1):
            ...
            if dis[i][j]==MAX:
                ...
                for k in range(len(pathList)):
                    # print('{}'.format(pathList[k]),end=' ')
                    outfile.write('->{}'.format(station[pathList[k]]))
                # print(' ')
                outfile.write('\n')

    outfile.close()

def printPath(path,start,target):#返回最短路径
    pathList=[];
