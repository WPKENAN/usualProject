...
class MODEL:
    def Vgg(width=64,height=64,depth=3,weight_decay=0.0005,classNum=0):
        ...
        def add_new_last_layer(base_model, classNum):
            ...
            x = Dense(500, activation='relu')(x)  # new FC layer, random init
            x=Dropout(0.5)(x)
...
def train(aug, trainX, trainY, testX, testY):
    ...
    model = MODEL.Vgg(norm_size, norm_size, depth, 0.0001, classNum=CLASS_NUM)
    model.summary()
    ...
    tb_cb = keras.callbacks.TensorBoard(log_dir="./vgglog", write_images=1, histogram_freq=0)
