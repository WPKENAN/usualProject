...
checkpointer = callbacks.ModelCheckpoint(filepath="kddresults/rnn/checkpoint-{epoch:02d}.hdf5", verbose=1, save_best_only=True, monitor='val_acc',mode='max')
...
model.save("kddresults/rnn/fullmodel/rnn_model.hdf5")

loss, accuracy = model.evaluate(X_test, y_test)
print("\nLoss: %.2f, Accuracy: %.2f%%" % (loss, accuracy*100))
y_pred = model.predict_classes(X_test)

np.savetxt('kddresults/rnn/rnn2predicted.txt', np.transpose(np.array([y_test.tolist(),y_pred.reshape(len(y_pred)).tolist()])), fmt='%01d')
