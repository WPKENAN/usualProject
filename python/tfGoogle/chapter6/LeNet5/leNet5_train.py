...
from tfGoogle.chapter6.LeNet5.leNet5_inference import inference,NUM_CHANNELS,IMAGE_SIZE,OUTPUT_NODE
from tfGoogle.chapter6.LeNet5 import leNet5_inference
BATCH_SIZE=100
...
def train(mnist):
    #调整输入数据的格式，输入为一个四维矩阵
    x=tf.placeholder(tf.float32,shape=(BATCH_SIZE,#第一个参数：一个batch里面的样例个数
                                       IMAGE_SIZE,#第二、三个参数：图片的尺寸
                                       IMAGE_SIZE,
                                       NUM_CHANNELS),name='x_input')#第四维表示图片的深度，对于RGB 深度是3
    y_=tf.placeholder(tf.float32,shape=(None,OUTPUT_NODE),name='y_input');

    #定义l2正则化
    regularizer=tf.contrib.layers.l2_regularizer(LAMADA);

    #计算神经网络的前向传播结果，注意训练的时候，dropout=True
    y=inference(x,False,True,regularizer);
    #定义存储模型训练轮数的变量，并指明为不可训练的参数
    global_step=tf.Variable(0,trainable=False);
    #初始化滑动平均的函数类，加入训练轮数的变量可以加快早期变量的更新速度
    variable_averages=tf.train.ExponentialMovingAverage(MOVING_AVERAGE_DECAY,global_step);
    #对神经网络里所有可训练的参数应用滑动平均模型，每次进行这个操作，列表里的元素都会得到更新
    variable_averages_op=variable_averages.apply(tf.trainable_variables());
    #计算使用了滑动平均的网络前向传播结果，滑动平均不会改变变量本身的值，而是维护一个影子变量记录滑动平均值
    #因此当需要使用滑动平均值时，需要明确调用average函数本身
    #训练的时候dropout=False,同时不计算正则化损失
...
if __name__ == '__main__':
    ...
    # leNet5_inference.__name__="__main__"
    # print(leNet5_inference.__name__)
    # print(leNet5_inference.__name__)
