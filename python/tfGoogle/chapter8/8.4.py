...
def generate_data(seq):
    ...
    #用sin函数前面的TIMESTEPS个点的信息，预测第i+TIMESTEPS个点的函数值
...
def lstm_model(X,y,is_training):
    ...
    #outputs是顶层LSTM再每一步的输出结果，他的维度是[batch_size,time,HIDDEN_SIZE]
    #在本问题中只关注最后一个时刻的结果
    output=outputs[:,-1,:]

    #对lstm网络的输出再加一层全连接层，并计算损失。这里的损失默认为平均损失
...
def run_eval(sess,test_X,test_y):
    ...
    with tf.variable_scope("model",reuse=True):
        prediction,_,_=lstm_model(X,None,None);
    ...
    #计算rmse作为评价指标
    predictions=np.array(predictions).squeeze()-0.1
