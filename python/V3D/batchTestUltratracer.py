    ...
    for file in os.listdir(inFolders[1]):
        if file[-3:] == 'swc':
            ...
            if os.path.exists(swcFiles[0]) and os.path.exists(swcFiles[1]) and os.path.exists(swcFiles[2]):
                print(swcFiles)
                shift_merge_Swc(swcFiles,outFolder+"\\"+file,99999,4000,0,0)
...
def renameSwc2(infolder,outfolder):
    for file in os.listdir(infolder):
        if file[-3:] == 'swc':
            shutil.copy(infolder+"\\"+file,outfolder+"\\"+str(int((file.split('.')[0]).split('_')[2]))+".swc")
...
def scaleFolder(infolder,outfolder,scale):
    for file in os.listdir(infolder):
        if file[-3:] == 'swc':
            scaleSwc(infolder+"\\"+file,outfolder+"\\"+file,2)
...
def main():
    #更改名称
    renameSwc1("D:\\soamdata\\ultratracer\\17302\\17302_app2\individual","D:\soamdata\\ultratracer\\17302\\app2");
    # renameSwc1("D:\\soamdata\\ultratracer\\17302\\17302_app3\\17302_multiTracer_ZZ", "D:\soamdata\\ultratracer\\17302\\app3");
    renameSwc2("D:\soamdata\\ultratracer\\17302\\17302_manual","D:\soamdata\\ultratracer\\17302\manual");

    #scale同一尺度
    app2Folder="D:\soamdata\\ultratracer\\17302\\app2"
    app3Folder="D:\soamdata\\ultratracer\\17302\\app3"
    manualFolder="D:\soamdata\\ultratracer\\17302\manual"
    mergeFolder="D:\soamdata\\ultratracer\\17302\merge_swc"
    app3_distance="D:\soamdata\\ultratracer\\17302\\app3_distance"
    app2_distance="D:\soamdata\\ultratracer\\17302\\app2_distance"
