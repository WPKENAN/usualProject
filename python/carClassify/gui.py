...
class App(tk.Frame):
    def __init__(self, master=None):
        ...
        ############################################################################################准备模型阶段,提高效率
        self.norm_size = train.norm_size
        self.labels_list = os.listdir("./image");
        self.labels_list.sort();
        print(self.labels_list)

        # load the trained convolutional neural network
        print("[INFO] loading network...")
        self.model = load_model("Vgg.hdf5")
        ################################################################################################
    ...
    def printcoords(self):
        try:
            self.File = filedialog.askopenfilename(parent=root, initialdir="./image", title='Choose an image.')
